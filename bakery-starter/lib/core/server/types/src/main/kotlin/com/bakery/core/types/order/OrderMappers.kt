...
fun DbOrder.orderLinesToDto(): OrderDto =
    OrderDto(
        id = id,
...
fun List<OrderWithLines>.orderLinesToDto(): OrderDto? {
    val group: Map<OrderDto, List<OrderWithLines>> =
        this.groupBy { row ->
            OrderDto(
                id = row.id,
    ...
    val dto: OrderDto? =
        group
            .map { (order: OrderDto, rows: List<OrderWithLines>) ->
                return@map OrderDto(
                    id = order.id,
...
fun List<OrderByUserWithLines>.orderByUserToDto(): OrderDto? {
    val group: Map<OrderDto, List<OrderByUserWithLines>> =
        this.groupBy { row ->
            OrderDto(
                id = row.id,
                totalAmount = row.total_amount,
    ...
    val dto: OrderDto? =
        group
            .map { (order: OrderDto, rows: List<OrderByUserWithLines>) ->
                return@map OrderDto(
                    id = order.id,
                    totalAmount = order.totalAmount,
                    paymentMethod = order.paymentMethod,
...
fun CreateOrderDto.toDb(totalAmount: Double): DbOrder =
    DbOrder(
...
fun CreateOrderDetailsDto.toDbDetails(
    orderId: String,
    totalPrice: Double,
): OrderLines =
    OrderLines(
...
fun CancelOrderDto.toUpdateDto(): UpdateOrderDto =
    UpdateOrderDto(
        id = id,
