...
class DefaultAuthStore(
    ...
    override suspend fun getValidUser(dto: SignInDto): AuthDto? {
        val dbUser: Bakery_user? =
            if (!verifyEmail(dto.username)) {
    ...
    override suspend fun forgotPassword(dto: ForgotPasswordDto): AuthDto? = null

    private suspend fun saveToken(userDto: UserDto): AuthDto? {
        val accessToken =
